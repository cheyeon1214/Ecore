...
import '../sell_donation_page/sell_product_form.dart'; // 글쓰기 페이지 import
...
class MyMarketProductpage extends StatelessWidget {
  ...
  Future<String> _getMarketName(String marketId) async {
    // Firestore에서 marketId에 해당하는 마켓 정보 가져오기
    DocumentSnapshot marketSnapshot =
    await FirebaseFirestore.instance.collection('Markets').doc(marketId).get();

    if (marketSnapshot.exists) {
      return marketSnapshot['name']; // name 필드를 가져옴
    } else {
      throw Exception('마켓을 찾을 수 없습니다.');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        StreamBuilder<List<SellPostModel>>(
        ...
        Positioned(
          ...
          child: FutureBuilder<String>(
            future: _getMarketName(marketId), // marketId를 이용해 마켓 이름 가져오기
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
              ...
              if (snapshot.hasData) {
                String marketName = snapshot.data!; // 마켓 이름 사용
                ...
                return FloatingActionButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => SellProductForm(name: marketName),  // 글쓰기 페이지로 마켓 이름 전달
                      ),
                    );
                  },
              ...
              return Container(); // 기본 반환값 (필요 시 추가 동작 설정 가능)
            },
          ),
        ),
      ],
